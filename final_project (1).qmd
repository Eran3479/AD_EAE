---
title: "ADS Final Project- Understanding User Behavior and Course Completion"
author: "Einat Liron 318808813<br> Assaf Nissim 12345456 <br> Eran Tsur 207350901"
format: 
  revealjs:
    smaller: true
    scrollable: true
...
The study focused on predicting course completion among two student groups: for-credit and open/MOOC students. It found that course completion was strongly influenced by engagement factors such as forum participation and the number of comments made by students. For-credit students' completion was more closely tied to these interactive elements, while MOOC students' completion was influenced by similar behaviors, though with slight variations in the predictive features. Overall, active engagement in course activities emerged as a key predictor of successful course completion for both groups.

## Factors for Success and Course Completion in Massive Open Online Courses through the Lens of Participant Types

The study offers valuable insights into factors affecting MOOC participants' success and course completion, highlighting the importance of participant categorization, interaction, and instructor effectiveness. By identifying participant types through diagnostic surveys and fostering interaction between students and instructors, MOOCs can enhance engagement and success rates.

However, the study is limited by its focus on a single platform and qualitative data, suggesting a need for future research incorporating quantitative data, cultural differences, and broader stakeholder perspectives. Overall, the findings provide a basis for designing more effective MOOCs and improving participant outcomes.

## article 5

## Conclusion from articles

The reviewed studies highlight that effective instructional design, personal motivation, and professional relevance are crucial for improving MOOC completion rates. Engaging course content, interactive elements, and alignment with participants' professional goals significantly enhance both engagement and success. Additionally, the length of the course and the type of assessment methods used are pivotal, with shorter courses and varied assessments generally leading to higher completion rates. Active participation in course activities is a strong predictor of success. Future research should address the limitations of current studies by incorporating quantitative data, exploring cultural differences, and considering diverse stakeholder perspectives to further enhance MOOC design and implementation.
...
```{r, echo=TRUE}
#_______ 1500 for each course category so it we have the same number of observations for each category______
set.seed(123)

data = data |>   # Subset 1500 records from each course category randomly
  group_by(CourseCategory) |>
  sample_n(1500)
...
#Calculate correlation for each course category between target variable (course completion) and the rest of the variables
cor_with_p = data |>
  group_by(CourseCategory) |>
...
```{r, echo=TRUE, results='hide'}
#A normality test for each column, separately for each CourseCategory,using the Shapiro-Wilk test, all of the results are < 0.05, meaning none of them follow a normal distribution
...
```{r, echo=TRUE, results='hide'}
# ______ Additional test for a normal distribution, plotting histograms________
...
We calculated the average Course Completion percentage across all course categories
...
Next, We calculated the average Course Completion percentage across all course categories by variables
...
We calculated the mean of the percentage of users for each device type within each course category to inspect its influence on course completion:
...
We chose a few parameters that we assumed the relationship between them affects **Course Completion.**
...
# Plot the most dense areas
ggplot(highdensity_combined3, aes(x = QuizScores, y = CompletionRate, color = Status)) +
  geom_point(alpha = 0.6) +
  facet_wrap(~ CourseCategory, ncol = 3) +
  scale_color_manual(values = c("Completed" = "blue", "Not Completed" = "red")) +
  labs(title = "High-Density Areas by Course Category",
...
# Plot the most dense areas
ggplot(highdensity_combined5, aes(x = NumberOfQuizzesTaken, y = QuizScores, color = Status)) +
  geom_point(alpha = 0.6) +
  facet_wrap(~ CourseCategory, ncol = 3) +
  scale_color_manual(values = c("Completed" = "blue", "Not Completed" = "red")) +
  labs(title = "High-Density Areas by Course Category",
...
# Plot the most dense areas
ggplot(highdensity_combined6, aes(x = TimeSpentOnCourse , y = QuizScores, color = Status)) +
  geom_point(alpha = 0.6) +
  facet_wrap(~ CourseCategory, ncol = 3) +
  scale_color_manual(values = c("Completed" = "blue", "Not Completed" = "red")) +
  labs(title = "High-Density Areas by Course Category",
...
● Anova: A method used to compare means across multiple groups to determine if there are significant differences among them.
...
Overall, **CompletionRate** consistently shows the strongest correlation with **CourseCompletion** across all categories, with values such as 0.328 in **Business** and 0.346 in **Health**. This suggests that **CompletionRate** is a key factor influencing course completion.

In addition to the correlation coefficients, the corresponding **P-values** are also presented, all of which are extremely low (\< 0.05) These low p-values indicate that the correlations are statistically significant, meaning that the relationships observed between the parameters and Course Completion are unlikely to be due to random chance.
...
For instance, in the **Business** category, the correlation between **NumberOfVideosWatched** and **TimeSpentOnCourse** is only 0.057, and in the **Health** category, **NumberOfVideosWatched** and **QuizScores** show a correlation of -0.047. These weak correlations indicate that the parameters, such as **TimeSpentOnCourse**, **NumberOfVideosWatched**, and **QuizScores**, are largely independent of one another within each course category, with minimal influence on one another.

```{r, echo=FALSE, results='asis', fig.width=10, fig.height=8}
# ___ A loop for the 5 matrices___

#___Matrices Visualization______
...
**Shapiro-Wilk normality test:** for examining normal distribution per parameter with each course category. The results give us an indication what statistical tests are appropriate for the distribution's type.

| Course category/ Parameter | Time spent on course | Number of videos watched | Number of quizzes taken | Quiz scores      | Completion rate  | Course completion |
|-----------|-----------|-----------|-----------|-----------|-----------|-----------|
| **Arts**                   | P- value \< 0.05     | P- value \< 0.05         | P- value \< 0.05        | P- value \< 0.05 | P- value \< 0.05 | P- value \< 0.05  |
| **Business**               | P- value \< 0.05     | P- value \< 0.05         | P- value \< 0.05        | P- value \< 0.05 | P- value \< 0.05 | P- value \< 0.05  |
| **Health**                 | P- value \< 0.05     | P- value \< 0.05         | P- value \< 0.05        | P- value \< 0.05 | P- value \< 0.05 | P- value \< 0.05  |
| **Programming**            | P- value \< 0.05     | P- value \< 0.05         | P- value \< 0.05        | P- value \< 0.05 | P- value \< 0.05 | P- value \< 0.05  |
| **Science**                | P- value \< 0.05     | P- value \< 0.05         | P- value \< 0.05        | P- value \< 0.05 | P- value \< 0.05 | P- value \< 0.05  |

All of the parameters have a p-value lower than 0.05, which means that **none of them are normally distributed.**

## Histogram Plotting:

We also plotted histograms to examine the distribution type further:
...
It can be observed that the shape of the histograms is **not typical of a normal distribution.**
...
In all course category the completing percentage is bellow 50 %, which is low. the course with the highest percentage is Business and the lowest is Health.
...
::: v-center-container
**Higher quiz scores** and **consistent course engagement** are the most reliable predictors of course completion across all categories. While time spent on the course and the number of quizzes taken are important, they are not sufficient on their own; students need to achieve scores **above 0.5** to increase their chances of completing the course successfully, particularly in the **Health** and **Programming** categories.
:::
...
Montgomery, C. (2020). *Person Studying Online*. Retrieved from <https://unsplash.com/photos/macbook-pro-displaying-group-of-people-smgTvepind4>. Unsplash License.

Romero-Rodríguez, L. M., Ramírez-Montoya, M. S., & Aguaded, I. (2020). Determining factors in MOOCs completion rates: Application test in energy sustainability courses. *Sustainability*, *12*(7), 2893.‏ <https://www.mdpi.com/2071-1050/12/7/2893>

Jordan, K. (2015). Massive open online course completion rates revisited: Assessment, length and attrition. *International Review of Research in Open and Distributed Learning*, *16*(3), 341-358. <https://www.erudit.org/en/journals/irrodl/2015-v16-n3-irrodl04980/1065985ar/abstract/>

Almeda, M. V., Zuech, J., Utz, C., Higgins, G., Reynolds, R., & Baker, R. S. (2018). Comparing the Factors That Predict Completion and Grades Among For-Credit and Open/MOOC Students in Online Learning. *Online Learning*, *22*(1), 1-18.‏ <https://eric.ed.gov/?id=EJ1179661>

Bingol, I., Kursun, E., & Kayaduman, H. (2020). Factors for success and course completion in massive open online courses through the lens of participant types. *Open Praxis*, *12*(2), 223-239.‏ <https://search.informit.org/doi/abs/10.3316/informit.352759430486694>
...
.reveal .toc .toc-item {
  display: block !important;  /* Ensure all TOC items are displayed */
}
